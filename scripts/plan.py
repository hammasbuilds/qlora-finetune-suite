...
KNOWN = {
    "qwen2.5-0.5b": ModelShape(
        ...
        intermediate_size=4864, vocab_size=151936, num_key_value_heads=2,
        family="qwen2", tie_word_embeddings=True,
    ),
    "qwen2.5-7b": ModelShape(
        "Qwen2.5-7B", hidden_size=3584, num_layers=28, num_attention_heads=28,
        intermediate_size=18944, vocab_size=152064, num_key_value_heads=4,
        family="qwen2",
    ),
    "llama3.1-8b": ModelShape(
        "Llama-3.1-8B", hidden_size=4096, num_layers=32, num_attention_heads=32,
        intermediate_size=14336, vocab_size=128256, num_key_value_heads=8,
        family="llama",
    ),
}
...
def main() -> None:
    ...
    for name, shape in KNOWN.items():
        ...
        print(f"  largest config fitting {VRAM_GB:.0f} GB: "
              f"r={best['r']}  batch={best['batch_size']}  "
              f"({best['estimated_gb']:.2f} GB, {best['headroom_gb']:.2f} GB spare)")

        report = plan_run(
            examples,
            TrainingConfig(r=best["r"], alpha=best["alpha"],
                           micro_batch_size=best["batch_size"]),
            shape=shape,
        )
        print("  plan: " + json.dumps(report.plan))
        print("  data: " + json.dumps(
            {k: report.data[k] for k in ("train", "eval", "supervised_fraction")}
        ))
